...
# 🛠️ FIX: nothing previously capped what a single container could consume -
# a runaway/malicious user script in one piper-runner-<lang> container could
# eat all host memory/CPU and starve every other container, including the
# orchestrator itself. Docker enforces these at the cgroup level, same
# mechanism Kubernetes' ResourceQuota/LimitRange ultimately compiles down to.
RUNNER_MEM_LIMIT = os.getenv("RUNNER_MEM_LIMIT", "512m")
RUNNER_CPU_LIMIT = float(os.getenv("RUNNER_CPU_LIMIT", "0.5"))
WORKER_MEM_LIMIT = os.getenv("WORKER_MEM_LIMIT", "1g")
WORKER_CPU_LIMIT = float(os.getenv("WORKER_CPU_LIMIT", "1.0"))
        ...
        try:
            ...
            client.containers.run(
                ...
                volumes={
                    ...
                    # 🛠️ FIX: docker exec (see executor.py's execute_in_sandbox) runs the
                    # user script from inside this container's own filesystem, so the
                    # script directory has to actually be mounted here - it never was
                    # before, which is why file_path lookups failed with ENOENT.
                    f"{clean_host_path}/external_script": {'bind': '/app/external_script', 'mode': 'ro'},
                },
                # 🛠️ FIX: this container no longer runs worker_daemon.py's Redis-stream
                # listener loop - executor.py now `docker exec`s straight into it per
                # task instead. All it needs to do is stay alive between execs, so a
                # keep-alive command replaces the old per-language interpreter command
                # (which, run with no args/stdin, would just exit immediately anyway).
