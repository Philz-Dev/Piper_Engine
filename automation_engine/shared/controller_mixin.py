...
class ControllerMixin:
    ...
    async def execute_operation(self, op: dict, context_env: dict):
        ...
        if action_name in ["goto", "break", "call", "retry", "continue", "execute", "exit", "ignore", "sleep"]:
    ...
    def handle_control_flow(self, action_name, op):
        ...
        if action_name == "exit": return {"signal": ActionSignal.EXIT}
        if action_name == "ignore": return {"signal": ActionSignal.IGNORE}
        if action_name == "sleep": return {"signal": ActionSignal.SLEEP}
        return None
