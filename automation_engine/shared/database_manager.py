...
class ContextDB:
    ...
    # meta_key this config lives under in engine_metadata, and the Redis key
    # watcher.py's get_runtime_config() polls - these two MUST match what's
    # configured there (CONFIG_REDIS_KEY, defaults to the same string).
    RUNTIME_CONFIG_META_KEY = "runtime_config"
    RUNTIME_CONFIG_REDIS_KEY = "piper:runtime_config"
    ...
    def _init_engine(self):
        ...
        # Best-effort Redis handle for the config write-through cache below.
        # DB stays the source of truth regardless of whether this connects -
        # a Redis outage should degrade watcher.py to its last-cached config,
        # never take down anything that writes through ContextDB.
        if ContextDB._redis is None:
            try:
                ContextDB._redis = redis.Redis(
                    host=os.getenv("REDIS_HOST", "redis-broker"),
                    port=int(os.getenv("REDIS_PORT", 6379)),
                    decode_responses=True,
                )
            except Exception as e:
                print(f"⚠️ Redis unavailable for config write-through cache: {e}")
                ContextDB._redis = None
        self.redis = ContextDB._redis
    ...
    def get_stored_ip(self):
        query = text("SELECT meta_value FROM engine_metadata WHERE meta_key = 'public_ip'")
        with self.engine.connect() as conn:
    ...
    def update_stored_ip(self, ip_address):
        query = text("""
            INSERT INTO engine_metadata (meta_key, meta_value, updated_at)
            VALUES ('public_ip', :ip, NOW())
            ON CONFLICT (meta_key) DO UPDATE SET meta_value = EXCLUDED.meta_value, updated_at = NOW();
        """)
        with self.engine.connect() as conn:
            conn.execute(query, {"ip": json.dumps(ip_address)})
            conn.commit()

    def get_runtime_config(self) -> dict:
        """
        Canonical read of the container/pool runtime config (max_workers,
        resource limits, chosen_languages, etc.) - Postgres is the source of
        truth. watcher.py itself never calls this directly (it only reads the
        Redis cache, to stay off the hot loop) - this is for whatever admin
        UI/API needs to display or edit current values.
        """
        query = text("SELECT meta_value FROM engine_metadata WHERE meta_key = :k")
        try:
            with self.engine.connect() as conn:
                result = conn.execute(query, {"k": self.RUNTIME_CONFIG_META_KEY}).fetchone()
                if result:
                    return result[0] if isinstance(result[0], dict) else json.loads(result[0])
                return {}
        except Exception as e:
            print(f"❌ Error fetching runtime config: {e}")
            return {}

    def update_runtime_config(self, config: dict) -> bool:
        """
        Upserts the runtime config into Postgres (source of truth), then
        write-throughs the same JSON into the Redis key watcher.py's
        get_runtime_config() polls - this is what makes a change here show up
        in the running controller within its cache TTL instead of needing a
        restart or a manual redis-cli SET.

        Only merges the keys passed in `config` with whatever's already
        stored, so a partial update (e.g. just {"max_workers": 3}) doesn't
        wipe out the rest of the config - callers don't have to resend every
        field every time.
        """
        ...
        try:
            with self.engine.connect() as conn:
                conn.execute(query, {"k": self.RUNTIME_CONFIG_META_KEY, "v": json.dumps(merged)})
                conn.commit()
        except Exception as e:
            print(f"❌ Error saving runtime config: {e}")
            return False

        # Best-effort cache write - the DB write above already committed and
        # is the durable source of truth, so a Redis hiccup here just means
        # watcher.py keeps serving its last-cached config until Redis comes
        # back, not that this update is lost.
        if self.redis:
            try:
                self.redis.set(self.RUNTIME_CONFIG_REDIS_KEY, json.dumps(merged))
            except Exception as e:
                print(f"⚠️ Runtime config saved to DB but Redis cache write failed: {e}")
