...
class ValidationState:
    ...
    depth: int = 0
    # Populated by import_validator: maps an import alias (the 'as:' value)
    # to the absolute path of the file it points to. use_validator reads
    # this to resolve 'use: <alias>.<id>' references.
...
class PiperRegistry:
    def __init__(self, definitions: Dict[str, Any]):
        ...
        self.partial_mandatory    = maps.get("partial_mandatory_map", {})
        #print(f"id:   {self.id_map["service"]}")

        # 3. Specialized Manager Maps (6 total)
        self.merger_map       = maps.get("merger_map", {})
        self.service_manager_map = maps.get("service_manager_map", {})
    ...
    def get_service_managers(self) -> Dict[str, Any]:
        """Returns all key-value pairs where is_service_manager is True."""
        return self.service_manager_map

    def is_service_manager_key(self, key: str) -> bool:
        """Returns True if the specific key acts as a service manager."""
        return self.service_manager_map.get(key, False)

    def get_partial_list(self, section_key):
        partial_list = self.partial_mandatory.get(section_key)
        
        if not partial_list:
            return []
        
        return [self.get_key_from_id(partial_key) for partial_key in partial_list]
