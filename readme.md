...
One script (`setup.sh`) provisions the entire stack: it generates credentials, writes a locked-down Nginx config, builds the Docker Compose topology on the fly, validates it, and brings everything up with health checks — all without requiring a pre-existing `docker-compose.yml`.
...
```bash
curl -fsSL [https://raw.githubusercontent.com/philz-dev/piper_engine/main/setup.sh](https://raw.githubusercontent.com/philz-dev/piper_engine/main/setup.sh) | bash
```
or
