...
async function initConsumerGroup() {
    while (true) {
        try {
            // Check if stream exists first, or create it with MKSTREAM
            await r.xgroup('CREATE', WORKER_STREAM, GROUP_NAME, '0', 'MKSTREAM');
            console.log(`✅ Created stream '${WORKER_STREAM}' and group '${GROUP_NAME}'.`);
            break;
        } catch (e) {
            if (e.message.includes('BUSYGROUP')) {
                ...
                console.log(`⚠️ Waiting for Redis stream/group initialization: ${e.message}`);
