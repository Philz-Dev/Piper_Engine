...
@app.post("/incoming/{client_name}/{task_id}/{app_name}")
async def mail_box(client_name: str, task_id: str, app_name: str, request: Request):
    try:
        ...
        # 1. Sync the incoming lead data to Context
        existing_context = db.get_context(client_name, task_id) or {}
        existing_context[app_name] = package
        db.save_context(client_name, task_id, existing_context)

        # 3. Add to Redis with the full blueprint context
        add_to_redis(
            dsl_name=client_name, 
            agency_id=task_id, 
            pipeline=pipeline_blueprint, # Pass the blueprint here
            from_trigger=True
        )
...
def start_webhook_service(port: int = 8003):
    ...
    max_retries = 20
    public_url = None
    added_url_val = "/{client_name}/{task_id}/{app_name}"
