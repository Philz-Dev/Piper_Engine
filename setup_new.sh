...
# =============================================================
# 🔌 DYNAMIC INJECTION: Write Central configuration variables into local .env
# =============================================================
write_or_update_env() {
    local key=$1
    local value=$2
    if grep -q "^$key=" .env; then
        # For Windows/Mac compatibility, we do a basic sed replace
...
write_or_update_env "INSTALL_TOKEN" "$INSTALL_TOKEN"
write_or_update_env "USER_EMAIL" "$USER_EMAIL"
write_or_update_env "USER_DOMAIN" "$USER_DOMAIN"
...
# Reload environment locally for this script execution
...
services:
  ...
  piper-servers:
    ...
    environment:
      ...
      - PYTHONPATH=/app
      - DOCKER_HOST=$INTERNAL_DOCKER_HOST
      - STRETIS_ENGINE_MODE=$ENGINE_MODE
      - NODE_ROOM_ID=$INSTALL_TOKEN
      - SIGNALING_SERVER_URL=$CENTRAL_API
