...
def int(value):
    # 🛠️ Was calling itself (int(value) inside def int) once this name
    # shadowed the builtin - RecursionError on every call. builtins.int
    # is the real conversion now; this function's own name is untouched.
    try:
...
def bool(value):
    if isinstance(value, builtins.bool): return value
...
def sum(*args) -> float:
    return builtins.sum([float(arg) for arg in args if arg is not None])

def round(value: float, precision: int = 0) -> float:
    return builtins.round(float(value), precision)
