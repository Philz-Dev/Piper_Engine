class UnZip:
    ...
    def unzip_content(self, content, current_path, parent=None, key=None, value=None, hooks=None):
        hook_func = None
        if hooks:
            if isinstance(content, dict):
                hook_func = hooks.get(dict)
            elif isinstance(content, list):
                hook_func = hooks.get(list)
            else:
                hook_func = hooks.get("primitive")
        ...
        if isinstance(content, dict):
            ...
            self.provision_leaf(content=content, parent=parent, current_path=current_path, key=key, hook=hooks)

    def unzip_dict(self, package: dict, current_path, hook=None, func=None, key=None):
        if func and key:
            func(content=package, value=package, key=key, path=current_path, parent=package)
        for k, v in package.items():
            # Build the new path segment
            new_path = f"{current_path}.{k}" if current_path else k
            self.unzip_content(content=v, current_path=new_path, key=k, hooks=hook, parent=package)

    def unzip_list(self, package: list, key, current_path, func=None, hook=None):
        value = {}
        if func and key:
            func(key=key, value=package, path=current_path, parent=package)
        for index, item in enumerate(package):
            ...
            self.unzip_content(content=item, current_path=new_path, key=None, hooks=hook, parent=package)
    
    def provision_leaf(self, content, key, parent, hook, current_path):
        ...
        if hook and "primitive" in hook:
            hook["primitive"](content=content, key=key, value={key: content}, path=current_path, parent=parent)